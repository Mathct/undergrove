...
class Champi43 extends Champi
{
    ...
    public function init($parg1, $parg2, $varg1, $varg2)
    {
        ...
        if ($parg1 != null)
        {
            ...
            undergrove::$instance->notifyAllPlayers("message",clienttranslate( '${player_name} activated ${name1} and copies ${name2}' ), array(
                ...
                'name2' => $this->listechampi[43]["name"],   //////////// changer le type
        ...
        {
            self::DbQuery( "UPDATE `champignon` set `carbone` = `carbone` + 1  WHERE `card_type` = 43" );        //////////// changer le type et nbre carbone
            undergrove::$instance->notifyAllPlayers("message",clienttranslate( '${player_name} activated ${name}' ), array(

                'i18n' => array( 'name'),
                'player_name' => $this->player_name,
                'name' => $this->listechampi[43]["name"],   //////////// changer le type
        ...
        $nbre = self::getUniqueValueFromDB("SELECT `nbre` FROM `champispecial` WHERE `type` = 43");
        if ($nbre == 4)
        {
        ...
        {
            undergrove::$instance->MajRessources();
            undergrove::$instance->addPendingTarget($this->player_id, "Champi43", "Champi43Step2");
        }
    ...
    public function argChampi43Step2($parg1, $parg2)
    {
        $ret = array();
        $ret["selectable"] = array();
        $ret['buttons'] = array();
    ...
    public function Champi43Step2($parg1, $parg2, $varg1, $varg2)
    {
        ...
        if ($varg1 == "1")
        {
            ...
            self::DbQuery( "UPDATE `champispecial` set `nbre` = `nbre` + 1  WHERE `type` = 43" );
        }

        if ($varg1 == "2")
        {
            
            self::DbQuery( "UPDATE `player` set `azote` = `azote` -2  WHERE `player_id` = {$this->player_id}" );
            self::DbQuery( "UPDATE `champispecial` set `nbre` = `nbre` + 2  WHERE `type` = 43" );
        }

        if ($varg1 == "3")
        {
            self::DbQuery( "UPDATE `player` set `azote` = `azote` -3  WHERE `player_id` = {$this->player_id}" );
            self::DbQuery( "UPDATE `champispecial` set `nbre` = `nbre` + 3  WHERE `type` = 43" );
        }

        $newnbre = self::getUniqueValueFromDB("SELECT `nbre` FROM `champispecial` WHERE `type` = 43");

        if ($newnbre == 1)
        {
            self::DbQuery( "UPDATE `champispecial` set `score` = 1  WHERE `type` = 43" );
        }

        if ($newnbre == 2)
        {
            self::DbQuery( "UPDATE `champispecial` set `score` = 2  WHERE `type` = 43" );
        }

        if ($newnbre == 3)
        {
            self::DbQuery( "UPDATE `champispecial` set `score` = 3  WHERE `type` = 43" );
        }

        if ($newnbre == 4)
        {
            self::DbQuery( "UPDATE `champispecial` set `score` = 5  WHERE `type` = 43" );
        }
...

        {
            ...
            $square = self::getUniqueValueFromDB("SELECT `card_location` FROM `champignon` WHERE `card_type` = 43");
            ...
            foreach ($racine as $controle)
            {
                ...
                $newscore = self::getUniqueValueFromDB("SELECT `score` FROM `champispecial` WHERE `type` = 43");
