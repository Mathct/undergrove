...
class Champi51 extends Champi
{
    public function arginit($parg1, $parg2)
    {
        ...
        $champi = self::getObjectListFromDB( "SELECT `card_id` `id`, `card_type` `type` FROM `champignon` WHERE `card_location` LIKE 'square%' AND `card_type` !=51 AND `card_type` !=1 AND (`card_type` <=27 or `card_type` >=33)");  
        
        foreach ($champi as $test)
        {
            if(($listechampi[$test['type']]["coutab"]==1) && ($listechampi[$test['type']]["coutc"] <= $this->player_carbone) && ($listechampi[$test['type']]["coutn"] <= $this->player_azote) && ($listechampi[$test['type']]["coutp"] <= $this->player_phosphore) && ($listechampi[$test['type']]["coutk"] <= $this->player_potassium) && ($listechampi[$test['type']]["coutlibre"] <= $this->player_azote+$this->player_phosphore+$this->player_potassium))
    ...
    public function init($parg1, $parg2, $varg1, $varg2)
    {
        ...
        $idcopieur = self::getUniqueValueFromDB("SELECT `card_id` FROM `champignon` WHERE `card_type` = 51");
